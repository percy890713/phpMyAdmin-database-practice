...
功能 4、5、6 皆支援**輸入 ID 或客戶姓名**兩種方式：

| 輸入內容 | 行為 |
|----------|------|
| 數字（如 `5`） | 直接用 ID 查詢 |
| 文字（如 `王小明`） | 用姓名搜尋，只有一筆直接操作；多筆則列出讓你選 ID |

**用 ID：**
```
請輸入訂單 id 或客戶姓名：5
  目前訂單：[5] 王小明 | 手機 x1 @ 18000元 | 2024-03-03 | 待處理
  狀態選項：待處理 / 已出貨 / 已完成
請輸入新狀態：已出貨
  ✓ 更新成功
```
...
請輸入訂單 id 或客戶姓名：王小明
  找到 2 筆訂單，請選擇要操作的：
  [5] 王小明 | 手機 x1 @ 18000元 | 2024-03-03 | 待處理
  [8] 王小明 | 鍵盤 x2 @ 1200元  | 2024-04-01 | 已出貨
  請輸入訂單 id：5
  狀態選項：待處理 / 已出貨 / 已完成
請輸入新狀態：已完成
  ✓ 更新成功
...
```
請輸入訂單 id 或客戶姓名：10
  即將刪除：[10] 王大明 | 手機 x1 @ 18000元 | 2024-03-03 | 待處理
  確認刪除？（y/n）：y
  ✓ 刪除成功
```
...
```
請輸入訂單 id 或客戶姓名：5
  目前訂單：[5] 王小明 | 手機 x1 @ 18000元 | 2024-03-03 | 已出貨
